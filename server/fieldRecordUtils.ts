...
/** Validate base64 document or image bytes uploaded to protected record APIs. */
export function decodeBase64Upload(base64: string): Buffer {
  const normalized = base64.replace(/\s/g, "");
  if (!normalized || normalized.length % 4 !== 0 || !/^[A-Za-z0-9+/]*={0,2}$/.test(normalized)) {
    throw new Error("파일 데이터 형식이 올바르지 않습니다.");
  }
  const buffer = Buffer.from(normalized, "base64");
  if (!buffer.length || buffer.length > MAX_FILE_UPLOAD_BYTES) {
    throw new Error("파일은 10MB 이하로 업로드해 주세요.");
  }
  return buffer;
}
